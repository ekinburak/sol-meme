 ...
 */
async function createNewToken(connection: Connection, payer: Keypair, mintAuthority: Keypair, freezeAuthority: Keypair | null): Promise<PublicKey> {
 ...
 */
async function mintTokens(connection: Connection, payer: Keypair, mint: PublicKey, recipient: PublicKey, mintAuthority: Keypair, amount: number): Promise<void> {
    await mintTo(
        connection,
        payer,
        mint,
 ...
 */
function saveKeypairToFile(keypair: Keypair, filename: string): void {
 ...
 */
async function findExistingTokenAccount(connection: Connection, mint: PublicKey, owner: PublicKey): Promise<PublicKey | null> {
...
async function createOrUpdateMetadata(umi: any, mintAddress: PublicKey, signer: any, metadata: any, initialize: boolean): Promise<void> {
...
async function main() {
    ...
    let tokenAccountInfo: any; // Declare the variable here
    ...
    const metadata = {
        ...
        symbol: "YTN",
        uri: "https://raw.githubusercontent.com/ekinburak/sol-meme/master/metadata.json", // Replace with your metadata URI
